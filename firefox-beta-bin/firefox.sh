...
# Defining variables
newsha64=`grep -w "linux-x86_64/en-US/firefox-$pkgver.tar.bz2" $sha | awk {'print $1'}`
newsha32=`grep -w "linux-i686/en-US/firefox-$pkgver.tar.bz2" $sha | awk {'print $1'}`
...
sleep 1
burp firefox-beta-bin-$pkgver-1.src.tar.gz
echo
